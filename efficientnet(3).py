...
from sklearn.metrics import precision_score, recall_score, accuracy_score, f1_score  # 导入精确率和召回率的计算函数
...
model_path = "0" #指定模型，0代表重新训练
read_rows = 3500  # 只读取数据前20行，输入0则全部读取
batch_size = 20  # 每个批次样本数
num_epochs = 10  # 训练周期数（测试时较少周期）
lr = 1e-4  # 学习率
...
def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs=25, device='cuda',save_interval=saved_interval, save_enabled=saved_enabled):
    ...
    for epoch in range(num_epochs):
        ...
        all_probs = 1.0 / (1.0 + np.exp(-all_outputs))
        all_pred_labels = (all_probs > 0.5).astype(int)

        print(f"预测值为\n{all_pred_labels}")
        print(f"真实值为\n{all_labels.astype(int)}")

        # 计算精确率和召回率（将所有标签展平后计算 micro 平均）
        accuracy = accuracy_score(all_labels.flatten(), all_pred_labels.flatten())
        ...
        f1 = f1_score(all_labels.flatten(), all_pred_labels.flatten(), zero_division=0)
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {epoch_loss:.4f}, Val Loss: {epoch_val_loss:.4f}, "
              f"Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}, Time: {epoch_time:.2f}s")
        # 调用保存模型函数
    ...
    plt.subplot(2, 2, 3)
    plt.plot(epochs, val_accuracies, label='Accuracy', marker='o', color='green')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    ...
    plt.subplot(2, 2, 4)
    plt.plot(epochs, val_f1s, label='F1 Score', marker='o', color='red')
    plt.xlabel('Epoch')
    plt.ylabel('F1 Score')
...
if __name__ == "__main__":
    ...
    # 开始训练并评估，返回模型及各项指标
    model, train_losses, val_losses, val_accuracies, val_precisions, val_recalls, val_f1s = train_model(
